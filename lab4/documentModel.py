...
class DocumentClassifier(nn.Module):
    def __init__(self, args, trainData, testData, word2vec, label2id, id2label, maxLen, stopWords):
    ...
    def train(self):
        ...
        for i in range(self.args.epoch):
            ...
            for i in range(self.args.label_cnt):
                ...
                f1[i] = 2*_precision*_recall/(_precision+_recall+1)
                f1Total += f1[i]
            ...
            if i % 5 == 0:
                acc = self.test()
            self.scheduler.step()
            print("第{}次训练，loss={:.4f} F1={:.4f} 学习率={:.7f}".format(i, lossVal, f1Total/self.args.label_cnt, self.scheduler.get_last_lr()[0]))
