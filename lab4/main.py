...
if args.task == 'DModel':
    ...
    if args.mode == 'train':
        ...
        # 初始化模型
        model = DocumentClassifier(args, trainDataSplited, testDataSplited, word2VecModel, label2id, id2label, args.maxLen, word2VecModel.stopWords)
        model.train()
    else:
    ...
    args.label_cnt = 288
    args.batch_size = 16
    trainData, testData = TData("./dataset/jena_climate_2009_2016.csv").read()
    model = TModel(args, trainData, testData)
    model.train()
